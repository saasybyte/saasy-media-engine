...
make vendor-deps      # slow path: builds libwebrtc + libmediasoupclient from source
                      # (tens of minutes on fast hardware, longer otherwise; the WebRTC source
                      # tree needs tens of GB of disk; sources are cached for reuse)
```
